...
function toPlatformApiAdminResponseUserMessage(
  error: PlatformApiAdminResponseError
): string {
  if (error.status === 409) {
    return error.detail ?? "A project with this slug already exists on Platform.";
  }
  if (error.status === 422 && error.detail) {
    return error.detail;
  }
  if (error.status === 401 || error.status === 403) {
    return "Platform API authorization failed. Check service credentials and scopes.";
  }
  if (error.status === 429) {
...
export function toPlatformProjectWriteUserMessage(error: unknown): string {
  ...
  if (error instanceof PlatformProjectBridgeError) {
    return error.message;
  }
  ...
  if (error instanceof PlatformApiAdminResponseError) {
    return toPlatformApiAdminResponseUserMessage(error);
  }
