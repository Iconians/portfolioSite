...
2. Move everything from `[src/app/Components/](src/app/Components/)` into the new tree; delete the old folder when done.
3. Update all imports project-wide (`@/app/Components/...` → `@/components/...`). `[@/`* maps to `./src/*](tsconfig.json)` — no tsconfig change required if imports use `@/components/...`.
4. Update `[next.config.ts](next.config.ts)` only if the webpack `resolve.alias` for `@` needs adjustment (today it points at `src`; keep consistent).
5. Run `bun run build` and smoke-test critical routes (/, /blogs, /admin, /About).
...
3. Keep API routes for external clients if desired; homepage no longer depends on them for first paint.
4. Revalidate: align with existing `[revalidate](src/app/page.tsx)` on the page.
